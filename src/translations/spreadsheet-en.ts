...
const spreadsheetEn = {
    ...
    'spreadsheet/custom_column/column_content_description': `Column content is described with variable names (in order to reference grid data), column IDs (in order to reference the content of another column), and operators provided by <Link>MathJS</Link> library (in order to transform grid data). A variable can also be prefixed with a node alias. Example: maxP - p in order to display active power reserve within the generator spreadsheet.`,
    ...
    'spreadsheet/custom_column/option/parameter': 'Parameter',
    'spreadsheet/custom_column/option/refresh': 'Reload',
    'spreadsheet/custom_column/option/refresh/tooltip':
        'Reload data for current sheet for other aliased nodes ({aliases})',
