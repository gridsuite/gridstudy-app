...
describe('services utils', function () {
        ...
        it('should handle missing Study metadata', async () => {
            mockFetch.mockImplementation(() =>
                Promise.resolve({
                    json: () =>
                        Promise.resolve([
                            { name: 'OtherApp', defaultParametersValues: {} },
                        ]),
                })
            );
    ...
    describe('toModificationOperation', () => {
        const valuesThatReturnObjectWithOpEqualToSET = [
            0,
            false,
            'truly value',
            1,
    ...
    describe('getRequestParamFromList', () => {
        ...
        it('should return empty URLSearchParams Object if the params array is empty', () => {
            expect(getRequestParamFromList([], 'mock-param')).toEqual(
                new URLSearchParams()
            );
        });
    });
    ...
    describe('getUrlWithToken', () => {
        ...
        it('should append the access_token parameter to the URL using "?" do not have params', () => {
            ...
            expect(getUrlWithToken(mockUrl)).toEqual(
                `${mockUrl}?access_token=${mockToken}`
            );
        });
    });
});
