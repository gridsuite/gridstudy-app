...
export const GeneratorRegulatingTerminalEditor = forwardRef(({ gridContext, colDef, gridApi, rowData }, ref) => {
    ...
    const handleSaveRegulatingTerminalPopup = (updatedRegulatedTerminal) => {
        const { equipment: { type: equipmentType, id: equipmentId } = {}, voltageLevel: { id: voltageLevelId } = {} } =
    ...
    return (
        <RegulatingTerminalModificationDialog
            ...
            onModifyRegulatingTerminalGenerator={(updatedRegulatedTerminal) => {
...
export const TWTRegulatingTerminalEditor = forwardRef(({ gridContext, colDef, gridApi, rowData }, ref) => {
    ...
    useImperativeHandle(
        ...
        () => {
            return {
                getValue: () => {
                    return getTapChangerRegulationTerminalValue(gridContext.dynamicValidation?.[tapChangerType] || {});
                },
                getField: () => {
                    return colDef.field;
                },
...
export const NumericalField = forwardRef(({ defaultValue, gridContext, colDef, gridApi, rowData }, ref) => {
    const error = useMemo(() => {
    ...
    const validateEvent = useCallback(
        (ev) => {
            ...
            setValue(newVal);
            gridContext.dynamicValidation = deepUpdateValue(gridContext.dynamicValidation, colDef.field, newVal);
...
export const BooleanListField = forwardRef(({ defaultValue, gridContext, colDef, gridApi }, ref) => {
    const intl = useIntl();
    const [value, setValue] = useState(defaultValue);
...
export const EnumListField = forwardRef(
    ({ defaultValue, enumOptions, gridContext, colDef, gridApi, stopEditing }, ref) => {
        const intl = useIntl();
        const [value, setValue] = useState(defaultValue);
