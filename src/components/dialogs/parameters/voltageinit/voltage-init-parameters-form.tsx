...
export function getVoltageInitParametersFormSchema(intl: IntlShape) {
    return yup.object().shape({
        [TabValue.GENERAL]: yup.object().shape({
            ...
            [REACTIVE_SLACKS_THRESHOLD]: yup
                .number()
                .min(0, intl.formatMessage({ id: 'ReactiveSlacksThresholdMustBeGreaterOrEqualToZero' }))
                .required(),
            [SHUNT_COMPENSATOR_ACTIVATION_THRESHOLD]: yup
                .number()
                .min(0, intl.formatMessage({ id: 'ShuntCompensatorActivationThresholdMustBeGreaterOrEqualToZero' }))
                .required(),
        }),
        ...
        [VOLTAGE_LIMITS_DEFAULT]: yup.array().of(
            yup.object().shape({
                ...
                [LOW_VOLTAGE_LIMIT]: yup
                    ...
                    .test(
                        (value, context) =>
                            !isBlankOrEmpty(value) || !isBlankOrEmpty(context.parent[HIGH_VOLTAGE_LIMIT])
                    ),
                [HIGH_VOLTAGE_LIMIT]: yup
                    ...
                    .nullable()
                    .test(
                        (value, context) => !isBlankOrEmpty(value) || !isBlankOrEmpty(context.parent[LOW_VOLTAGE_LIMIT])
                    ),
...
export type VoltageInitParametersForm = InferType<ReturnType<typeof getVoltageInitParametersFormSchema>>;
