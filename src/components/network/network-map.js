...
const NetworkMap = (props) => {
    ...
    function onViewStateChange(info) {
        // Fix for navigator zooming: in this case label visibility should be changed only at the end of the animation
        // to avoid interrupting the animation

        // 2 ways for zooming
        //   - using wheel mouse: there is no animation and interactionState.isZooming is defined
        //   - using navigator: zooming is done using an animation, first viewState does not have interactionState field
        //                      and viewState.zoom contains the target zoom. Next viewState are generated by the animation
        //                      until reaching the target zoom.
        if (info.interactionState) {
            if (info.interactionState.isZooming // wheel zoom case
                || info.viewState.zoom === targetZoom) { // last viewState of the navigator animation case
            ...
            // navigator initial animation viewState, we store the target zoom
            targetZoom = info.viewState.zoom;
        }
    }
