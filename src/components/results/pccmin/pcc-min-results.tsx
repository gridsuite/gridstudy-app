...
import { PccMinResultProps } from './pcc-min-result.type';

export const PccMinResults: FunctionComponent<PccMinResultProps> = ({
    result,
    isLoadingResult,
    columnDefs,
    tableName,
    ...
    const openLoaderTab = useOpenLoaderShortWait({
        isLoading:
            ...
            PccMinStatus === RunningStatus.RUNNING ||
    ...
    const renderPccMinResults = () => {
        const message = getNoRowsMessage(messages, result.singlePccMinResultInfos, PccMinStatus, !isLoadingResult);
        const rowsToShow = getRows(result.singlePccMinResultInfos, PccMinStatus);
