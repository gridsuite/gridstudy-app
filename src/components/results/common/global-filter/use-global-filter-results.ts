...
export function useGlobalFilterResults(filters: GlobalFilter[], equipmentTypes: NonEmptyTuple<FilterEquipmentType>) {
    ...
    const fetchFilteredIds = useCallback(
        (filtersParam: GlobalFilter[], equipmentTypesParam: NonEmptyTuple<FilterEquipmentType>) => {
            const globalFilters = canEvaluate ? buildValidGlobalFilters(filtersParam) : undefined;
            if (!globalFilters || !studyUuid || !currentNode?.id || !currentRootNetworkUuid) {
                // nothing to evaluate: don't leave the caller waiting for ids that will never come
                setIsPending(false);
                return;
            }
            evaluateGlobalFilter(studyUuid, currentNode.id, currentRootNetworkUuid, equipmentTypesParam, globalFilters)
                .then(setFilteredIds)
                .catch((error) => {
        ...
        [canEvaluate, currentNode?.id, currentRootNetworkUuid, snackError, studyUuid]
    );

    const debouncedFetchFilteredIds = useDebounce(fetchFilteredIds);

    useEffect(() => {
        // pending as soon as the filters change, until the debounced evaluation has resolved
        setIsPending(canEvaluate && filters.length > 0);
        debouncedFetchFilteredIds(filters, equipmentTypes);
    }, [canEvaluate, debouncedFetchFilteredIds, equipmentTypes, filters]);
